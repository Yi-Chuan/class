...
// echo $str= '你好,这里是卖咖啡!';
// echo '<br />';
// echo iconv('UTF-8','GB2312', $str); //将字符串的编码从GB2312转到UTF-8
...
/*	
	...
	// 2,选择库
	mysqli_select_db($link,'test') or die('数据库选中失败');
	//3,设置字符集
	mysqli_set_charset($link,'utf8') or die('字符集失败');
...
// 1). mysqli_query($link,"insert into user(name) values('test')");
// 		$id = mysqli_insert_id($link);
// 		Mysqli_query($link,"insert into score(uid,subjext,score) values(".$id.",'english','99')");


// (2)	获取score表里uid为2的用户score最高的5条记录 （4分）
// 		select uid,sunjext,score from score where uid=2 
// 		order by score desc limit 0,5;

// (3)	使用联合查询获取name为"张三"的用户的总分数	（3分）
	// select s.score from score s RIGHT JOIN user u 
	// 	ON u.id=s.uid where u.name='张三';

// (4)	删除name为"李四"的用户，包括分数记录 （3分）
	// delete from score where uid in(select id from user where name='李四');
