...
//10.数据类型(int char varchar datetime text)的意思;请问varchar和char有什么别？（3分）
char:定长 /效率高
varchar:不定长/ 效率低

// 2、echo(),print(),print_r()的区别(3分)
echo和print是php结构print_r是函数,语句没有返回值,函数可以有返回值(即便没有用)  
...
include
  PHP在遇到include命令时它就必须重新解释一次，如果在同一个PHP网页中出现10次include它便会被重新解释10次
require
  PHP遇到require不管它在同一个PHP网页中出现过几次，PHP只会解释一次而已
  所以当它在同一个PHP网页中解释过一次后，第二次出现便不会再解释，这是它的优点。
  不过严格来说，这也是它的唯一的缺点，
  因为它不会重复解释引入的文件所以当PHP网页中使用循环或条件语句来引入文件时，require则不会做任何的改变。
  当有类似这样的情形时，就必须使用include命令来引入文件了  
  require的工作方式是为了让PHP程序得到更高的效率  
...
普通传值：函数范围内对值的任何改变在函数外部都会被忽略
优缺点：php必须复制值。
    特别是对于大型的字符串和对象来说，
    这将会是一个代价很大的操作。
引用传值：函数范围内对值的任何改变在函数外部也能反映出这些修改
优缺点：
  按引用传递则不需要复制值，
  对于性能提高很有好处。
...
// 20写一个函数，尽可能高效的，从一个标准 url 里取出文件的扩展名
function getExt($url){
  pathinfo($url);
}
//写一个函数，算出两个文件的相对路径 　　（5分）
//如 $a = '/a/b/c/d/e.php'; $b = '/a/b/12/34/c.php';
$a='/a/b/m/c/d/i/e.php';
$b='/a/b/m/f/g/n/c.php';//  a相对于b  ../../../f/g/n/c.php
function getPath($a,$b){
  $filename=pathinfo($b)['basename'];
  $a1= explode('/',dirname($a));
  $b1= explode('/',dirname($b));
...
function read($path){
  $files=array();//文件数组
  $path=opendir($path);
  if($filename=@readdir($path)){
    if($filename=='.' || $filename=='..'){
        continue;
    }
    if(is_dir($filename)){
      $files[$filename]=read($path.'/'.$filename);
    }else{
